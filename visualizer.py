...
def render_pptx_slides(pptx_path: str, cache_dir: str, prefix: str = "slide") -> List[str]:
    """
    Render PPTX to images using LibreOffice + PyMuPDF.
    Gracefully falls back to returning an empty list if tools are missing.
    """
    pdf_path = convert_pptx_to_pdf(pptx_path, cache_dir)
    if not pdf_path:
        warnings.warn(f"Skipping visual extraction for {pptx_path} (LibreOffice not found).")
        return []
